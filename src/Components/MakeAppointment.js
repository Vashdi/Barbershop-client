...
const MakeAppointment = (props) => {
  ...
  const language = "he";
  const dense = false;
  const [newStrict, setNewStrict] = useState([
    { before: new Date() },
    {
      after: new Date(
  ...
  useEffect(() => {
    const getAtStartClosedDays = async () => {
      try {
        ...
        const closedDaysWithoutID = closedDaysArray.map(
          (closedDay) => new Date(closedDay.date)
        );
    ...
    const start = async () => {
      try {
        ...
        stricts.forEach((strict) => {
          ...
          newStrictsToShow = newStrictsToShow.concat(
  ...
  useEffect(() => {
    ...
    const sortedAppointments =
      appointmentService.sortAppointments(allAppointments);
  ...
  useEffect(() => {
    const whenLogged = async () => {
      try {
        ...
        const sortedAppointments =
          appointmentService.sortAppointments(allAppointments);
        const appointmentsToShowByString =
          appointmentService.stringAppointments(sortedAppointments);
  ...
  const addAppointment = async () => {
    if (myHour === "pick a time.." || selectedDay === " ") {
      ...
      try {
        ...
        const resp = await appService.create(appointment, hoursToShow, () => {
          setModifiers({
            ...modifiers,
            closedDays: modifiers.closedDays.concat([selectedDay]),
          });
        });
      ...
      <div className="appointmentContainer">
        ...
        {storeData.AppointmentReducer.step === 2 && (
          <div className="sum">
            ...
            <h2 className="hourTitle" style={{ marginBottom: "40px" }}>
              התור שנבחר
            </h2>
            <h4 className="hourTitle">:תאריך</h4>
            <h4 className="hourTitle">
              {selectedDay.getDate() +
                "/" +
            ...
            <h4 className="hourTitle">{myHour}</h4>
            <br />
            ...
            <Fade in={openModal}>
              <div className={classesModal.paper}>
                <h2 id="transition-modal-title">
                  <p className="clientNextApp"> התורים הקרובים שלי</p>
                </h2>
                        ...
                        return (
                          <SingleAppointment
                            ...
                            callback={(day) => {
                              setModifiers({
                                ...modifiers,
                                closedDays: modifiers.closedDays.filter(
                                  (date) =>
